...
app.get('/trek_character', async(req, res) => {
  try {
    const data = await client.query(`
          SELECT  tc.id, tc.name, tc.species, tf.faction AS faction, tc.rank, tc.is_carbon_based
...
app.get('/trek_character/:id', async(req, res) => {
  try {
    const data = await client.query(`SELECT * from trek_character where id=${req.params.id}`);
